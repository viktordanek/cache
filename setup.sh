...
then
  ...
  declare ${HASH_ENVIRONMENT_VARIABLE}=$( ${ECHO} ${PRE_HASH} $(( ${!TIMESTAMP_ENVIRONMENT_VARIABLE} / ${LIFETIME} )) | ${SHA512SUM} | ${CUT} --bytes 128 ) &&
  ...
  then
    ...
    then
      ...
      ${LN} --symbolic ${RESOURCE} ${RESOURCES}/${!HASH_ENVIRONMENT_VARIABLE}/resource &&
